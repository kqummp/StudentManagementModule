...
usrmgr.Delete = async function (reserve_id, session_user) {
  ...
  let result;
  try {
    result = await usrmgr.QueryByReserveId(reserve_id, session_user);
  } catch (err) {
    throw err;
  }
  ...
  if (!(result.message === message.success && result.user === session_user)) {
...
usrmgr.Modify = async function (reserve_id, operation, session_user, data) {
  ...
  try {
    delete_result = await usrmgr.Delete(reserve_id, session_user);
  } catch (err) {
    throw err;
  }
  ...
  if (operation) {
    let insert_result;
    try {
      insert_result = await usrmgr.Book(data, session_user);
    } catch (err) {
      throw err;
    }
...
usrmgr.Status = async function (reserve_id, session_user) {
  if (typeof reserve_id === "undefined" || reserve_id === "") {
    throw new Error(message.invalid_field);
  }
